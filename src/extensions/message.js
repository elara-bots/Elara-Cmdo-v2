...
const functions = {
	blacklist: async (client, message) => {
		try{
			...
			console.log(`[Blacklist, Check] | Error`, err.stack)
			return false;
		}
    },
	...
	maint: (client) => {
		try{
			if(client.main === true) return true;
			return false;
		}catch(err){
			console.log(`[Maintenance, Check] | Error`, err.stack);
			return false;
		}
	}
}
...
module.exports = Structures.extend('Message', Message => {
	...
	class CommandoMessage extends Message {
		...
		async run() { // eslint-disable-line complexity
			...
			if(await functions.blacklist(this.client, this) === true){ // If they are in the blacklist user array.
				this.client.emit('commandBlock', this, "blacklist");
				return this.command.onBlock(this, "blacklist")
			};
			if(functions.maint(this.client) === true && !this.client.isOwner(this.author)){ // If the bot is in maintenance mode.
